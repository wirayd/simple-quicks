...
folder are mapped to the application's root namespace, 'SimpleQuicks'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "SimpleQuicks"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'SimpleQuicks' in the same way
as `"SimpleQuicks/sass/src"`.
